 ...
 * @version v1.4.1 - 2016-02-26 * @link https://github.com/jacobscarter/angular-cron-jobs
...
angular.module('angular-cron-jobs').factory('cronService', function() {
    ...
    service.fromCron = function(value) {
        ...
        } else if(cron[1] === '*' && (cron[2] === '*' || cron[2] === '?') && cron[3] === '*'  && cron[4] === '*') {
            frequency.base = 2; // every hour
        } else if((cron[2] === '*' || cron[2] === '?') && cron[3] === '*'  && cron[4] === '*') {
            frequency.base = 3; // every day
        } else if((cron[2] === '*' || cron[2] === '?') && cron[3] === '*') {
            frequency.base = 4; // every week
        } else if(cron[3] === '*' && (cron[4] === '*' || cron[4] === '?')) {
